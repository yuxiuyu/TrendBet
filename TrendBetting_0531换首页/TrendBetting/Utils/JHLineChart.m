...
- (void)configPerXAndPerY{
    ...
    switch (_lineChartQuadrantType) {
        case JHLineChartQuadrantTypeFirstQuardrant:
        {
            ...
            {
                _perXLen = (_xLength-kXandYSpaceForSuperView)/(_xLineDataArr.count);
            }
...
/**
 *  重写ValueArr的setter方法 赋值时改变x轴刻度大小
 *
 */
-(void)setXLineDataArr:(NSArray *)xLineDataArr
{
    NSMutableArray*showX=[[NSMutableArray alloc]init];
    [showX addObject:@"0"];
    [showX addObjectsFromArray:xLineDataArr];
    _xLineDataArr=showX;
}
/**
...
-(void)setValueArr:(NSArray *)valueArr{
    ...
    for (int i=0; i<valueArr.count; i++)
    {
        NSMutableArray*showX=[[NSMutableArray alloc]init];
        [showX addObject:@"0"];
        [showX addObjectsFromArray:valueArr[i]];
        [allArr addObject:showX];
    }
    _valueArr=allArr;;
...
#warning yxyedit
                    ...
                    {
                        NSLog(@"%f  %f",_perXLen,self.chartOrigin.x);
...
- (void)drawPositionLineWithContext:(CGContextRef)context{
    ...
    for (NSInteger m = 0;m<_valueArr.count;m++)
    {
        ...
        {
            for (int a=yxyadd; a<yxyarr.count; a++)
            {
                if ([yxyarr[a] length]>0)
                {
                    yxyadd=a;
                    break;
                }
            ...
            if (p.y!=0) {
#warning yxy edit
                UIColor *pointNumberColor = (_pointNumberColorArr.count >= _valueArr.count?(_pointNumberColorArr[m]):([UIColor blackColor]));
                ...
                switch (_lineChartQuadrantType) {
                    ...
                    case JHLineChartQuadrantTypeFirstAndFouthQuardrant:
                    {
//                        NSString *aimStr = [NSString stringWithFormat:@"(%@,%@)",_xLineDataArr[yxyadd],_valueArr[m][yxyadd]];
                         NSString *aimStr = [NSString stringWithFormat:@"%@",_valueArr[m][yxyadd]];
                        yxyadd++;
                        CGSize size = [self sizeOfStringWithMaxSize:CGSizeMake(100, 25) textFont:self.valueFontSize aimString:aimStr];
                        CGFloat length = size.width;
                        [self drawText:aimStr andContext:context atPoint:P_M(p.x - length / 2, p.y - size.height / 2 -5) WithColor:pointNumberColor andFontSize:self.valueFontSize];
