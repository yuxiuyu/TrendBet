...
- (void)drawRect:(CGRect)rect {
    ...
    for (int i = 0; i < self.xTitleArray.count; i++) {
        ...
        CGRect titleRect = CGRectMake((i + 1) * self.pointGap - labelSize.width / 2,(self.frame.size.height - labelSize.height+50)/2,labelSize.width,labelSize.height);
        ...
        if (i == 0) {
            ...
            //画垂直X轴的竖线kChartLineColor
            [self drawLine:context
                startPoint:CGPointMake(titleRect.origin.x+labelSize.width/2, (self.frame.size.height+40)/2)
                  endPoint:CGPointMake(titleRect.origin.x+labelSize.width/2, (self.frame.size.height+30)/2)
    ...
    CGFloat offSet = self.yMax + self.yMin;
    CGPoint startPoint =CGPointMake(0, (self.frame.size.height+34)/2);
    
//    if (offSet>0 && offSet<self.yMax) {
//        startPoint = CGPointMake(0, ((self.frame.size.height+40)/2)+(((self.frame.size.height+40)/2)*offSet)/((self.yMax+fabs(self.yMin))/5));
//    }
    
    // CGPointMake(0, (self.frame.size.height+40)/2)
    [self drawLine:context
        startPoint:CGPointMake(0, (self.frame.size.height+40)/2)
          endPoint:CGPointMake(self.frame.size.width, (self.frame.size.height+40)/2)
         lineColor:kChartLineColor
         lineWidth:1];
    ...
    //////////////// 画最底部的分割线 /////////////////////////
    NSDictionary *bottomattribute = @{NSFontAttributeName : [UIFont systemFontOfSize:8]};
    CGSize bottomtextSize = [@"x" sizeWithAttributes:bottomattribute];


    [self drawLine:context
        startPoint:CGPointMake(0, self.frame.size.height-10)
          endPoint:CGPointMake(self.frame.size.width, self.frame.size.height-10)
         lineColor:kChartLineColor
         lineWidth:1];
